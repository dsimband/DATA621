...
tmp_df <- train_df %>% filter(TARGET_WINS < 40)
t_df <- melt(tmp_df)

t_df %>% ggplot(aes(x= value)) + 
    geom_density(fill='gray') + facet_wrap(~variable, scales = 'free') 
...
Consider dropping teams with less than 20 wins
...
sk_df$p_missing <- round(sk_df$n_missing / data_rows(sk_df),4)
sk_df %>% dplyr::select(skim_type,skim_variable,n_missing,p_missing) %>% filter(n_missing > 0) %>% dplyr::arrange(desc(p_missing))

print(paste0('Number of complete records: ',nrow(train_df %>% drop_na())))
...
ft <- lm(TARGET_WINS ~ TEAM_BASERUN_CS, data=train_df)
summary(ft)

plot(TARGET_WINS ~ TEAM_BASERUN_CS, data=train_df)
abline(ft, col='blue')
...
data.frame(M) %>% filter(TARGET_WINS > 0.1 | TARGET_WINS < -0.1) %>% dplyr::select(TARGET_WINS)


M <- cor(trainFix_df)
M1 <- data.frame(M) %>% filter(TARGET_WINS > 0.1 | TARGET_WINS < -0.1) %>% dplyr::select(TARGET_WINS)

print('Columns with greater abs(0.1) correlation: ')
print(rownames(M1))


columnNames <- rownames(M1)
...
#trainFix_df %>% explore(columnNames = TARGET_WINS, split = TRUE)

#trainFix_df %>% explain_tree(target = TARGET_WINS)

# df %>% select(age,bmi,children,charges) %>% pairs(panel = panel.smooth, gap=0.5)

#trainFix_df %>% explain_tree(target = TARGET_WINS)
trainFix_df %>% dplyr::select(columnNames) %>% pairs(panel = panel.smooth, gap=0.5)
...
#ggplot(trainFix_df, aes(x=log(TEAM_BATTING_H), y=TARGET_WINS, color=na_flag)) + geom_point()
ggplot(trainFix_df, aes(x=TEAM_BATTING_H, y=TARGET_WINS, color=TEAM_BATTING_2B)) + geom_point()
ggplot(trainFix_df, aes(x=TEAM_BATTING_H, y=TARGET_WINS, color=TEAM_BATTING_BB)) + geom_point()

ggplot(trainFix_df, aes(x=TEAM_BATTING_2B, y=TARGET_WINS, color=na_flag)) + geom_point()

ggplot(trainFix_df, aes(x=TEAM_BATTING_BB, y=TARGET_WINS, color=na_flag)) + geom_point()
...
The dataframe was updated in a previous step to support the data exporation steps. 

- trainFix_df - substitutes the average of a variable for all NA values
- trainOmit_df - removes the columns with NA values

```{r}

skim(train_df) %>% dplyr::select(skim_variable,n_missing)
skim(trainFix_df) %>% dplyr::select(skim_variable,n_missing)
skim(trainOmit_df) %>% dplyr::select(skim_variable,n_missing)
